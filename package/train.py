...
def train(args=None,trial_run=True):
    ...
    loaded_train_dataset = data_utils.Dataloader(dataset = train_dataset,
                            dataset_size = len(train_dataset.img_path),
                            batch_size = 29,
                            shuffle = True
                            )

    loaded_valid_dataset = data_utils.Dataloader(dataset = valid_dataset,
                                dataset_size = len(valid_dataset.img_path),
                                batch_size = 29,
                                shuffle = False
                                )

    loaded_test_dataset = data_utils.Dataloader(dataset = test_dataset,
                                dataset_size = len(test_dataset.img_path),
                                batch_size = 29,
                                shuffle = False
                                )

    model = VGG16_Unet(n_classes=3)
    model.compile(optimizer=tf.keras.optimizers.Adam(0.0001),
                    loss=DiceLoss(),
                    metrics=['accuracy'])

    callbacks = [
        tf.keras.callbacks.ModelCheckpoint('Checkpoint.h5', save_best_only=True, mode='min'),
